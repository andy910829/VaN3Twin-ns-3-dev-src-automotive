...
class emergencyVehicleAlert : public Application
{
  ...
  private:
    /**
    * @brief 用於週期性檢查煞車狀態的事件。
    */
    EventId m_checkBrakeEvent;

    /**
    * @brief 儲存上一個時間點的速度，用於計算加速度。
    */
    double m_previousSpeed;

    /**
    * @brief 緊急煞車的減速度閾值 (m/s^2)，例如 -4 m/s^2。
    * 注意：加速度是負值。
    */
    const double EMERGENCY_BRAKE_THRESHOLD = -4.0;

    /**
    * @brief 週期性檢查車輛是否正在緊急煞車。
    */
    void CheckAndSendEmergencyBrakeDenm();
    ...
    EventId m_rangeCheckEvent; //定期重置顏色

    bool m_send_cam;
    bool m_send_cpm;
    bool m_send_denm = false;
